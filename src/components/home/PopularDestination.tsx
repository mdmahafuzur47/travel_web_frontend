        ...
        <section className='container my-20'>
            <div>
                <div className='flex flex-col gap-3'>
                    <p className='text-primaryColor font-medium text-xl'>Destination</p>
                    <h1 className='text-4xl tracking-wider font-medium'>Popular destination</h1>
                    <div className='bg-primaryColor w-[130px] h-[1.5px]'>
                    </div>
                </div>
                ...
                <div className='grid grid-cols-4 mt-8 gap-5'>
                    {
                        destinationData?.map((data, index) => {
                            return <PopularDestinationCard index={index} key={index} data={data} />
                        })
                    }
                </div>
            </div>
...
const PopularDestinationCard = ({ data, index }: { data: IDestination, index: number }) => {
    return (
        <div className={`rounded-2xl relative h-full group overflow-hidden shadow-sm w-full ${index % 2 !== 0 && "lg:mt-8"}`}>
            <img className='w-full group-hover:scale-110 transition-all duration-500  h-full rounded-2xl object-cover' src={data.img} alt={data.name} />
